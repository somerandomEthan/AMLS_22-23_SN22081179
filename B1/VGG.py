...
def extract_images_labels(final_image_dir, cartoon_images_dir, labels_filename):
    ...
    individual_labels = dict((line.split('\t')[3].strip('\n') , int(line.split('\t')[2])) for line in lines[1:])# the label for face shape is located at the third column
    ...
    return images_datasets, faceshape_labels
...
def create_VGG19_model(optimizer='adam', learn_rate=0.001, amsgrad=False,):
    num_classes = 5
    base_model = VGG19(include_top=False, weights='imagenet', input_tensor=None, input_shape=(128, 128, 3), pooling=None, classifier_activation='softmax')
